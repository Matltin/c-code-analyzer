...
این راهنما برای نسخه‌ی کامل فاز سوم و Bonus زیرساختی نوشته شده و فرض می‌کند ترمینال در ریشه‌ی
...
نکته‌ی دقیق: کد تمام ۲۵۸ تست خودکار فعلی در جریان پیاده‌سازی پروژه توسط من
...
### Bonus زیرساختی — ۱۴ تست جدید

| فایل | تعداد | پوشش | منبع سناریو |
|---|---:|---|---|
| `tests/test_bonus/test_docker_infrastructure.py` | 6 | Stageها، User غیرـRoot، Context و Makefile | [هر دو] |
| `tests/test_bonus/test_ci_pages_infrastructure.py` | 8 | Coverage، CI، Permission، Pages، Site و Docs | [هر دو] |

جمع این بخش: `14` تست.

جمع کل: `154 + 90 + 14 = 258` تست.
...
- Coverage خط و Branch با Gate حداقل ۸۰٪.
- Docker چندمرحله‌ای، Runtime غیرـRoot و Test Stage.
- CI روی Python 3.10 و 3.12 و Artifactها.
- Site محلی و Workflow انتشار GitHub Pages.
...
تست‌های Docker در Repository ساختار را بررسی می‌کنند، اما جای اجرای واقعی
`make bonus-docker` روی سیستم دارای Docker را نمی‌گیرند. Workflowها نیز باید
بعد از Push در GitHub اجرا شوند.
...
فاز سوم با اجرای محلی کاربر تأیید شده است. برای بررسی Bonus زیرساختی این
دستورها را اجرا کن:
...
- `258 passed`
- `0 failed`
- Coverage ترکیبی حداقل `80%`
- بازشدن `site/index.html` و لینک‌های داخلی
- Build، Help، Version، Smoke و Test Stage موفق Docker
- سبزشدن CI و Pages پس از Push در GitHub
- Compile بدون خطا

تا قبل از اجرای واقعی Docker و GitHub، B2، B3 و Pages در Checklist باید `[?]`
باقی بمانند؛ موفقیت تست ساختاری به‌تنهایی تأیید آن سرویس‌ها نیست.
